...
class LRPanelImpl @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : SwipeRefreshLayout(context, attrs), LoadRefreshPanel {
    ...
    override fun render(vs: LRViewState<*>) {
        when {
            vs.loading -> LR_ViewAnimator.showIfNotYet(LAYER_LOADING)
            vs.loadingError != null -> LR_ViewAnimator.showIfNotYet(LAYER_ERROR)
            else -> LR_ViewAnimator.showIfNotYet(LAYER_MODEL)
        }

        if (this.isRefreshing != vs.refreshing) this.isRefreshing = vs.refreshing
        if (this.isEnabled != vs.canRefresh) this.isEnabled = vs.canRefresh
    }
}