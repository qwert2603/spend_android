...
import com.qwert2603.spenddemo.utils.toSqlTime
...
class RemoteDBFacade @Inject constructor(
    ...
    fun getSpends(lastUpdateMillis: Timestamp = Timestamp(0), lastUpdatedId: Long = 0, limit: Int = 20): List<RemoteSpend> = remoteDB.query(
            "SELECT id, kind, value, date, time, updated, deleted" +
    ...
    fun insertSpend(spend: Spend): Long = remoteDB.query(
            "INSERT INTO ${E.env.remoteTableNameSpends} (kind, value, date, time) VALUES (?, ?, ?, ?) returning id",
            { IdSqlWrapper(it).id },
            listOf(spend.kind, spend.value, spend.date.toSqlDate(), spend.time?.toSqlTime().asNullableArg(Types.TIME))
    ).single()

    fun updateSpend(spend: Spend) {
        remoteDB.execute(
                "UPDATE ${E.env.remoteTableNameSpends} SET kind=?, value=?, date=?, time=?, updated=NOW() WHERE id=?",
                listOf(spend.kind, spend.value, spend.date.toSqlDate(), spend.time?.toSqlTime().asNullableArg(Types.TIME), spend.id)
        )
    }
    ...
    fun insertProfit(profit: Profit): Long = remoteDB.query(
            "INSERT INTO ${E.env.remoteTableNameProfits} (kind, value, date, time) VALUES (?, ?, ?, ?) returning id",
            { IdSqlWrapper(it).id },
            listOf(profit.kind, profit.value, profit.date.toSqlDate(), profit.time?.toSqlTime().asNullableArg(Types.TIME))
    ).single()

    fun updateProfit(profit: Profit) {
        remoteDB.execute(
                "UPDATE ${E.env.remoteTableNameProfits} SET kind=?, value=?, date=?, time=?, updated=NOW() WHERE id=?",
                listOf(profit.kind, profit.value, profit.date.toSqlDate(), profit.time?.toSqlTime().asNullableArg(Types.TIME), profit.id)
        )
    }
